...
function dbConnect($config){
	try {
		...
		$host_opion = 'mysql:host='    . $config['db']['host'].';
...
//($dbcon, ['id','password','email'], 'cn_user', 'username', $userInfo['username']);
	try {
        ...
        $inKeys = array_map(function($key){return ':var_'.$key;},       array_keys($xzParam));
...
//echo $sql."\n";
        $data = $dbcon->prepare($sql);
	    foreach($xzParam as $key=>$val) {
//echo $key."  ".$val."\n";
        $data->bindParam(':var_'.$key, $val);
        }
		$data->execute(); 
...
function isSessionActive($dbcon){
	if (isset($_SESSION['auth_user']) && $_SESSION['auth_user']!=''){
		$result = getSelect($dbcon, ['login'], 'user', 'id', $_SESSION['auth_user'], "LIMIT 1");
		if ($result || empty ($result)) {
			if (count($result)!=0){
